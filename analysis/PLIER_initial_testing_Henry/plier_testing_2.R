### Purpose of this script is to figure out how plier is selecting the number of LVs
# From playing with the PLIER source code, it was learned that the number of LVs
# Is determined by SVD with the elbow plot method -- the number from this 
# analysis is multiplied by 2 and that, or 90% of samples (whichever number is greater) is used as k.
...
plierResult <- PLIER(dataWholeBlood, allPaths, seed = 3)
...
PLIER=function(data, priorMat,svdres=NULL, k=NULL, L1=NULL, L2=NULL, L3=NULL, 
               frac=0.7,  max.iter=350, trace=F, scale=T, Chat=NULL, maxPath=10,
               doCrossval=T, penalty.factor=rep(1,ncol(priorMat)), glm_alpha=0.9,
               minGenes=10, tol=1e-6, seed=123456, allGenes=F, rseed=NULL, 
  ...
  if(is.null(Chat)){
    Cp=crossprod(C)
    Chat=PLIER:::pinv.ridge(crossprod(C), 5)%*%(t(C))
  }
  YsqSum=sum(Y^2)
  ...
  for ( i in 1:max.iter){
    ...
    if(i>=iter.full.start){
      if(i==iter.full & !L3.given){ #update L3 to the target fraction
        Ulist=PLIER:::solveU(Z, Chat, C, penalty.factor, pathwaySelection, glm_alpha, maxPath, target.frac = frac)
        U=Ulist$U
        L3=Ulist$L3
        ...
        U=PLIER:::solveU(Z, Chat, C, penalty.factor, pathwaySelection, glm_alpha, maxPath, L3=L3)
      }
  ...
  if(doCrossval){
    outAUC=PLIER:::crossVal(out, Y, priorMat, priorMatCV)
  } else{
    message("Not using cross-validation. AUCs and p-values may be over-optimistic")
    outAUC=PLIER:::getAUC(out, Y, priorMat)
  }
